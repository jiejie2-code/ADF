...
from net_2 import Restormer_Encoder, Restormer_Decoder, BaseFeatureExtraction, DetailFeatureExtraction, AFF, iAFF
from net_2 import BasicLayer
...
optimizer5 = torch.optim.Adam(
    FeatureB_iAFF.parameters(), lr=lr, weight_decay=weight_decay)
optimizer6 = torch.optim.Adam(
    FeatureD_iAFF.parameters(), lr=lr, weight_decay=weight_decay)
...
# data loader
trainloader = DataLoader(H5Dataset(r"data/MSRS_train_1_imgsize_128_stride_200.h5"),
...
for epoch in range(num_epochs):
    ...
    for i, (data_VIS, data_IR) in enumerate(loader['train']):
        ...
        FeatureB_iAFF.zero_grad()
        FeatureD_iAFF.zero_grad()
        ...
        if epoch < epoch_gap:  # Phase I
            ...
            feature_A_B = FeatureB_iAFF(feature_V_B, feature_I_B)
            feature_F_D_1 = FeatureD_iAFF(feature_I_D, feature_V_D)

            B, C, W, H = feature_A_B.shape  # 获取输入的形状
            feature_A_B = feature_A_B.view(B, C, -1)  # 将 W 和 H 维度合并成一个维度
            ...
            nn.utils.clip_grad_norm_(
                FeatureB_iAFF.parameters(), max_norm=clip_grad_norm_value, norm_type=2)
            nn.utils.clip_grad_norm_(
                FeatureD_iAFF.parameters(), max_norm=clip_grad_norm_value, norm_type=2)
...
if True:
    checkpoint = {
        ...
        'FeatureB_iAFF': FeatureB_iAFF.state_dict(),
        'FeatureD_iAFF': FeatureD_iAFF.state_dict(),
    }
    torch.save(checkpoint, os.path.join("models/CDDFuse_Final1" + timestamp + '.pth'))

# print(DIDF_Encoder)
# print(DIDF_Decoder)
# print(BaseFuseLayer)
# print(DetailFuseLayer)
